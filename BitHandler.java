...
/** BitHandler receives bits and sends them to
 *  panel, a LightPanel which reads the bits from the
 *  socket. A BitHandler is able to broadcast bits to
...
public class BitHandler extends Thread {
	...
	/** The default constructor for BitHandler.
	 *  Initializes the hosts to 'localhost' and
	 *  the port to the LightSystem's default port.
	 */
	public BitHandler() {
		this("localhost", LightSystem.DEFAULT_PORT);
	}

	/** An expicit constructor for BitHandler.
	 * @param host  The host of the LightPanel
	 * @param port  The port of the LightPanel
	 */
	...
	/** pause(milliseconds) stops the thread for the 
	...
	/** notifyReceived(bits) sends the bits that have been received
	 *  to the BitListener
	 * @param bits  The bits that were received.
	 */
